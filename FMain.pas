...
uses
  ...
  Registry, StrUtils, Menus, AppEvnts, ComCtrls, ComObj, TLHelp32, PsAPI, RpCon,
  RpConDS, RpConBDE, RpRender, RpRenderPDF, RpBase, RpSystem, RpDefine, RpRave,
  RpRenderCanvas, RpRenderPreview, AlexUtils, PngImage, sSkinManager, sSkinProvider,
  PngSpeedButton, ToolWin, acCoolBar, sComboBox, sPanel, sSpeedButton;
...
type
  ...
  TMainForm = class(TForm)
    ...
    btnUnBlock: TPngSpeedButton;
    btnBlock: TPngSpeedButton;
    btnSelect: TPngSpeedButton;
    btnBalanta: TPngSpeedButton;
    btnPrint: TPngSpeedButton;
    btnJournals: TPngSpeedButton;
    btnMail: TPngSpeedButton;
    btnSent: TPngSpeedButton;
    btnExport: TPngSpeedButton;
    btnAccount: TPngSpeedButton;
    btnSendAccount: TPngSpeedButton;
    pUp: TsCoolBar;
    LCopyRight: TLabel;
    pTools: TsPanel;
    LDBGest: TLabel;
    btnFilter: TPngSpeedButton;
    LGrupuri: TLabel;
    btnMFilter: TPngSpeedButton;
    cbFirme: TsComboBox;
    CGroups: TsComboBox;
    pDown: TsPanel;
    IDown: TImage;
    ...
    btnSesizari: TsSpeedButton;
    btnPassword: TsSpeedButton;
    btnDate: TsSpeedButton;
    btnVerify: TsSpeedButton;
    btnFiltru2: TsSpeedButton;
    btnCuBlocati: TsSpeedButton;
    btnZero: TsSpeedButton;
    btnContract: TsSpeedButton;
    ...
    procedure IDownOverClick(Sender: TObject);
    procedure IDownOverMouseLeave(Sender: TObject);
    procedure IUpOverClick(Sender: TObject);
    procedure IUpOverMouseLeave(Sender: TObject);
    procedure IDownMouseEnter(Sender: TObject);
    procedure IUpMouseEnter(Sender: TObject);
    procedure ImgPanelMouseEnter(Sender: TObject);
    procedure LPanelMouseEnter(Sender: TObject);
    procedure pbSettingsPaint(Sender: TObject);
    procedure IFilterOMouseLeave(Sender: TObject);
    procedure cbFirmeChange(Sender: TObject);
    procedure TimerTimer(Sender: TObject);
    procedure FormMouseUp(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    procedure ForceVerify(Sender: TObject);
    procedure LCopyRightMouseEnter(Sender: TObject);
    procedure LCopyRightMouseLeave(Sender: TObject);
    procedure btnExitClick(Sender: TObject);
    procedure btnSettingsClick(Sender: TObject);
    procedure btnContractClick(Sender: TObject);
    procedure btnZeroClick(Sender: TObject);
    procedure btnCuBlocatiClick(Sender: TObject);
  private
...
uses Color, Fur_Detail, Fisa_Client, E_Mail, Send_Mail, Temp_Rap, Extras_Cont,
  ...
  ListareFacturi, Bank, Editare, Sent_Mail, Updater, Test;
...
begin
  ...
  DoSQL(MyQ, 'SELECT DESCRIERE FROM `FDSOFT`.`GENERAL` WHERE COD = "BTTIP_FUR"');
  //MyQ.SQL.Text := 'SELECT DESCRIERE FROM `FDSOFT`.`GENERAL` WHERE COD = "BTTIP_FUR"';
  //MyQ.Active := True;
  if not MyQ.Active then Exit;
...
begin            //1-Misu, 0-Eu nou
  if DBManager.FieldCount = 0 then Exit;
...
var
  ...
  Temp: integer;
begin
  if DBGest.FieldCount = 0 then Exit;
  Temp := 0;
  for I := 0 to 8 do
...
begin
  if btnContract.Down then begin
    btnContract.Down := True;
    Options.Values['CU_CONTRACT'] := 'TRUE';
    btnContract.Font.Style := [fsBold];
  end else begin
    btnContract.Down := False;
    Options.Values['CU_CONTRACT'] := 'FALSE';
    btnContract.Font.Style := [];
  end;
  SetFilter;
  CGestChange(Self);
end;

procedure TMainForm.btnZeroClick(Sender: TObject);
begin
  if btnZero.Down then begin
    btnZero.Down := True;
    Options.Values['CU_ZERO'] := 'TRUE';
    btnContract.Font.Style := [fsBold];
  end else begin
    btnZero.Down := False;
    Options.Values['CU_ZERO'] := 'FALSE';
    btnZero.Font.Style := [];
  end;
  SetFilter;
  CGestChange(Self);
...
procedure TMainForm.btnCuBlocatiClick(Sender: TObject);
begin
  if btnCuBlocati.Down then begin
    btnCuBlocati.Down := True;
    Options.Values['CU_BLOCATI'] := 'TRUE';
    btnCuBlocati.Font.Style := [fsBold];
  end else begin
    btnCuBlocati.Down := False;
    Options.Values['CU_BLOCATI'] := 'FALSE';
    btnCuBlocati.Font.Style := [];
  end;
  EventMapper(Copy((Sender as TComponent).Name, 4));
end;
...
begin                         
  ...
  if (DBGest.Tag <> 0)  then begin FDialog.Start('Atentie', 'Selectati firma pe care doriti sa blocati clientul!.', fdOk, mtWarning); Exit; end;
  ...
  if FDialog.Start('Atentie', 'Sunteti sigur(a) ca doriti asta?', fdBtn_Yes or fdBtn_No, mtConfirmation) = fdBtn_Yes then begin
    ...
    //log
    QTemp.SQL.Text := 
      'INSERT INTO `DBCABCONTA`.`LOG` (FIRMA, CODFUR, ACTION, DATETIME, USER) ' + 
        'SELECT "' + CGest.Text + '", CODFUR, "B", NOW(), "' + Copy(UMail, 1, Pos('@', UMail) - 1) + '" ' + 
        'FROM `' + CGest.Text + '`.`FUR` WHERE CODFUR IN (' + StringReplace('"' + StringReplace(Selections.CommaText, ',', '","', [rfReplaceAll]) + '")', '""', '"', [rfReplaceAll]);
    QTemp.ExecSQL;
    //\log
...
begin
  if (Selections.Count = 0) then begin FDialog.Start('Atentie', 'Nu ati selectat nimic.', fdOk, mtWarning); Exit; end;
  if (DBGest.Tag <> 0)  then begin FDialog.Start('Atentie', 'Selectati firma pe care doriti sa deblocati clientul!.', fdOk, mtWarning); Exit; end;
  ...
  if FDialog.Start('Atentie', 'Sunteti sigur(a) ca doriti asta?', fdBtn_Yes or fdBtn_No, mtConfirmation) = fdBtn_Yes then begin
    ...
    QTemp.SQL.Text := 
      'INSERT INTO `DBCABCONTA`.`LOG` (FIRMA, CODFUR, ACTION, DATETIME, USER) ' + 
        'SELECT "' + CGest.Text + '", CODFUR, "D", NOW(), "' + Copy(UMail, 1, Pos('@', UMail) - 1) + '" ' + 
        'FROM `' + CGest.Text + '`.`FUR` WHERE CODFUR IN (' + StringReplace('"' + StringReplace(Selections.CommaText, ',', '","', [rfReplaceAll]) + '")', '""', '"', [rfReplaceAll]);
    QTemp.ExecSQL;
    //\log
    for I := 0 to Selections.Count - 1 do begin
...
begin
  ...
  for I := 0 to Selections.Count - 1 do begin
    ...
    with ExtrasCont do begin
      ...
      GetDetails;
      L1.Text := Copy(Temp, 1, 2);
      A1.Text := '20' + Copy(Temp, 3, 2);
...
begin
  ...
  TempSQL := 'SELECT DENFUR, CODFC, SECTIA, NRFACT, SUMA FROM (SELECT CODFC, COUNT(CODFC) AS NRFACT, ' +
             ...
             'GROUP BY CODFC) AS TEMP LEFT JOIN `' + CGest.Text + '`.`FUR` ON CODFUR=CODFC WHERE (SUMA <> 0) AND (NRFACT * CLASA >= ' + Options.Values['NR_FACT'] +') AND ' +
  ...
  if QGest.Tag = 1 then begin
    ...
    UnionAll := 'UNION ALL ' +
                ...
                'WHERE (CODFUR NOT IN ' +
                  ...
                  'WHERE ((SUMADB-SUMACR) <> 0) AND (CONTFC = "4111")) ' +
    ...
    btnFilter.Font.Style := [];
  end
  else begin
    QGest.Tag := 1;
//    if HavingFact = '<SHIFT>' then
    HavingFact := iif(HavingFact = '<SHIFT>','HAVING SECTIA = "B"','HAVING (COUNT(CODFUR) * CLASA >=' + Options.Values['NR_FACT'] + ') ');
//    HavingFact := 'HAVING (COUNT(CODFUR) * CLASA >=' + Options.Values['NR_FACT'] + ') ';
    //QGest.Filter := '[#] >= ' + Options.Values['NR_FACT'];
    //QGest.Filtered := True;
  ...
  LGest.Caption := IntToStr(QTemp.RecordCount) + '/' + IntToStr(QGest.RecordCount) + ' înregisträri.';
...
begin
  ...
  //Aux := QManager.Fields[0].Text + '|' + QManager.FieldByName('ID').Text + '$' + QManager.FieldByName('DENUMIRE').Text + '!' + QManager.FieldByName('EMAIL').Text
      ...
      ManagerResize;
      DBManager.Tag := 1;
      LManager.Caption := IntToStr(QManager.RecordCount) + ' înregisträri.';
      LMSelected.Caption := IntToStr(MSelections.Count) + ' selectate.';
      //LMSelected.Left := DBManager.Left + DBManager.Width - LMSelected.Width;
      LManager.Caption := IntToStr(QManager.RecordCount) + ' înregisträri.';
      btnMFilter.Font.Style := [fsBold];
    end;
  end else begin //Misu  ///////////////////////////////////////
    if DBManager.Tag = 1 then begin
...
begin
  ...
  Selections.Clear;
  while not QGest.Eof do begin
    if QGest.Fields[4].AsInteger >= StrToInt(Options.Values['NR_FACT']) then
...
procedure TMainForm.ForceVerify(Sender: TObject);
var
  Temp: integer;
begin
  Temp := DoVerify;
...
begin
  CGest.ItemIndex := cbFirme.ItemIndex;
  CGestChange(Self);
end;
...
begin
  //if CBContract.Checked then Contract := 'AND ((CONT_USD <> "") OR (CONT_DM <> ""))' //deprecated
  if Options.Values['CU_CONTRACT'] = 'TRUE' then Contract := 'AND ((CONT_USD <> "") OR (CONT_DM <> ""))'
  else Contract := '';
  if (Options.Values['CU_ZERO'] = 'TRUE') and (QGest.Tag <> 1) then
    UnionAll := 'UNION ALL ' +
                'SELECT CODFUR AS `Cod Fiscal`, DENFUR AS `Denumire`, CONCAT(CONT_DM, CONT_USD), ' +
                'SECTIA AS `St.`, 0 AS `#`, 0 AS `Sold`, " " AS `Data`, CLASA AS `L`, CONCAT(IF(FTIP&2=2,"C",""), ' +
                  ...
                  'IF(FTIP&64=64,"Gw",""), IF(FTIP&128=128,"Rc",""), IF(FTIP&256=256,"Ex",""), IF(FTIP&512=512,"Gr",""), IF(FTIP&1024=1024,"Bk","")) AS `Co` ' +
              //  '" " AS DATE_DOC ' +
                'FROM `' + CGest.Text + '`.`FUR` ' +
                'WHERE (CODFUR NOT IN ' +
                  '(SELECT CODFC FROM `' + CGest.Text + '`.`FRES' + SDate + '` ' +
...
begin
  if Now - LastActionStamp < 50 then //daca s-a executat o actiune in ultimele 50 minute
    DoSchedule('T');
    DoSchedule('UA');
  //  ShowMessage('Act');
  //ShowMessage(DateTimeToStr(LastActionStamp) + ':' + IntToStr(Trunc((Now - LastActionStamp)*24*60)));
end;
...
begin
  ...
  GestSQL := 'SELECT CODFUR AS `Cod Fiscal`, DENFUR AS `Denumire`, CONTRACT AS `$`, SECTIA AS `St.`, ' +
             ...
             'CLASA AS `I`, ' +
             'CONCAT(IF(FTIP&2=2,"C",""), IF(FTIP&4=4,"G",""), IF(FTIP&8=8,"S",""), IF(FTIP&16=16,"M",""), IF(FTIP&32=32,"F",""), ' +
               'IF(FTIP&64=64,"Gw",""), IF(FTIP&128=128,"Rc",""), IF(FTIP&256=256,"Ex",""), IF(FTIP&512=512,"Gr",""), IF(FTIP&1024=1024,"Bk","")) AS `Co` ' +
            // 'GROUP_CONCAT("Nr. ", NRDFC, " / ", DATA,  " - ", SUMA, " RON" ORDER BY DATADOC) AS DATE_DOC ' +
             ...
             UnionAll;
  if DoSQL(QGest, GestSQL) = -1 then Exit;
  //QGest.SQL.Text := GestSQL;
  //QGest.Active := True;
  TempSQL :=
    'SELECT DENFUR, CODFC, SECTIA, NRFACT, SUMA ' +
    'FROM (' +
      'SELECT CODFC, COUNT(CODFC) AS NRFACT, SUM(SUMADB-SUMACR) AS SUMA ' +
      'FROM `' + CGest.Text + '`.`FRES' + SDate + '` ' +
      'WHERE ((SUMADB-SUMACR < -10) OR (SUMADB-SUMACR > 10)) AND (CONTFC = "4111") ' +
      'GROUP BY CODFC) AS TEMP ' +
      'LEFT JOIN `' + CGest.Text + '`.`FUR` ON CODFUR=CODFC ' +
    'WHERE (SUMA <> 0) AND (NRFACT >= ' + Options.Values['NR_FACT'] +') AND ((CONT_USD <> "") OR (CONT_DM <> "" )) ' +
    'ORDER BY SECTIA, DENFUR';
  DoSQL(QTemp, TempSQL);
  //QTemp.SQL.Text := TempSQL;
  //QTemp.Active := True;
  QGestRec := QGest.RecordCount;
  ...
  if QGest.Filtered then begin
    TempSQL := 'SELECT DENFUR, CODFC, SECTIA, NRFACT, SUMA FROM (SELECT CODFC, COUNT(CODFC) AS NRFACT, ' +
               'SUM(SUMADB-SUMACR) AS SUMA FROM `' + CGest.Text + '`.`FRES' + SDate + '` WHERE ((SUMADB-SUMACR < -10) OR (SUMADB-SUMACR > 10)) AND (CONTFC = "4111") ' +
               'GROUP BY CODFC) AS TEMP LEFT JOIN `' + CGest.Text + '`.`FUR` ON CODFUR=CODFC WHERE (SUMA <> 0) AND (NRFACT >= ' + Options.Values['NR_FACT'] +') AND ' +
               '((CONT_USD <> "") OR (CONT_DM <> "" )) ORDER BY SECTIA, DENFUR';
    DoSQL(QTemp, 'SELECT SUM(`NRFACT`), SUM(SUMA) FROM (' + TempSQL + ') AS TEMP2');
    //QTemp.SQL.Text := 'SELECT SUM(`NRFACT`), SUM(SUMA) FROM (' + TempSQL + ') AS TEMP2';
    //QTemp.Active := True;
    if QTemp.RecordCount <> 0 then
      LFact.Caption := 'Facturi neachitate: ' + QTemp.Fields[0].AsString;
  end else begin
    DoSQL(QTemp, 'SELECT SUM(`#`), SUM(Sold) FROM (' + GestSQL + ') AS TEMP2');
    //QTemp.SQL.Text := 'SELECT SUM(`#`), SUM(Sold) FROM (' + GestSQL + ') AS TEMP2';
    //QTemp.Active := True;
...
begin
  ...
  if CGroups.ItemIndex = 0 then begin
    ...
    if Options.Values['CU_BLOCATI'] = 'TRUE' then CuBlocati := ''
...
begin
  QManager.Tag := 0;
  if DoSQL(QManager, AManagerSQL) = -1 then Exit;
  //QManager.SQL.Text := AManagerSQL;
  //QManager.Active := True;
  //QTemp.Active := False;
  ManagerResize; //Resize
  MSelections.Clear;
  if QManager.RecordCount <> 0 then begin
    LManager.Caption := IntToStr(QManager.RecordCount) + ' înregisträri.';
    QManagerRec := QManager.RecordCount;
  end;
  LMSelected.Caption := IntToStr(MSelections.Count) + ' selectate.';
...
begin
  ...
  if (Sender as TDBGrid).Name = 'DBGest' then begin
    Ovr := QGest.FieldByName('St.').AsString = 'B';
    ...
    if (Ovr) then begin
      (Sender as TDBGrid).Canvas.Font.Color := Colors.Blocked;
      (Sender as TDBGrid).Canvas.Font.Style := [fsBold];
    end;
    
    if (Field.Index = 2) and (S <> '') then begin //Contracte
//      (Sender as TDBGrid).Canvas.Brush.Color := Colors.Contract;//$00C7FADA;
      (Sender as TDBGrid).Canvas.Font.Color := Colors.Contract;
      (Sender as TDBGrid).Canvas.Font.Style := [fsBold];
    end;

    if (Field.Index = 3) and (S = 'B') then begin
//      (Sender as TDBGrid).Canvas.Brush.Color := Colors.Blocked;//$00A0A5FF;
      (Sender as TDBGrid).Canvas.Font.Color := Colors.Blocked;//$00A0A5FF;
      (Sender as TDBGrid).Canvas.Font.Style := [fsBold];
      Ovr := True;
    end;
    if (Field.Index = 4) then begin
      if(StrToInt(S) >= StrToInt(Options.Values['NR_FACT'])) then begin
//        (Sender as TDBGrid).Canvas.Brush.Color := Colors.Overdue;//$00A0A5FF;
        (Sender as TDBGrid).Canvas.Font.Color := Colors.Overdue;//$00A0A5FF;
        (Sender as TDBGrid).Canvas.Font.Style := [fsBold];
        Ovr := True;
      end;
    ...
    if Field.Index = 5 then begin
      if S = '0' then (Sender as TDBGrid).Canvas.Font.Color := MedianColor;
      try S := FloatToStrF(s2f(S),ffFixed, 15,2); except  end;
      if Ovr then
//        (Sender as TDBGrid).Canvas.Brush.Color := Colors.Overdue;//$00A0A5FF;
        (Sender as TDBGrid).Canvas.Font.Color := Colors.Overdue;//$00A0A5FF;
      Alignment := 2;
    end;
    if Field.Index = 6 then Alignment := 1;
    if (TDBGrid(Sender).Tag = 2) and (Field.Index = 8) then begin
      if Pos('F', S) <> 0 then
        (Sender as TDBGrid).Canvas.Font.Color := $00A6FFFF;
      if (Pos('D', S) <> 0) then begin
        if (QGest.FieldByName('St.').AsString = 'B') then
          (Sender as TDBGrid).Canvas.Font.Color := Colors.Overdue
        else
          S := '';  
      end;
    end;
  ...
  DrawText((Sender as TDBGrid).Canvas.Handle, PChar(S), Length(S), ARect, Alignment);
end;
    ...
    if Selections.IndexOf(Aux) >= 0 then begin
      ...
      QTemp.SQL.Text :=
        ...
        'WHERE (ABS(SUMADB - SUMACR ) > ' + FloatToStr(Treshold) +
          ') AND (CODFC = "' + DBGest.Columns[0].Field.AsString + '") AND (CONTFC = "4111")';
      QTemp.Open;
    ...
    if EGestCauta.Tag <> 112 then begin DBGest.SetFocus; Exit; end
      ...
      for I := 0 to CGest.Items.Count - 1 do
        GestSQL := GestSQL +
                   ...
                   'CLASA AS `I`, ' +
                   'CONCAT(IF(FTIP&2=2,"C",""), IF(FTIP&4=4,"G",""), IF(FTIP&8=8,"S",""), IF(FTIP&16=16,"M",""), IF(FTIP&32=32,"F",""), ' +
                     'IF(FTIP&64=64,"Gw",""), IF(FTIP&128=128,"Rc",""), IF(FTIP&256=256,"Ex",""), IF(FTIP&512=512,"Gr",""), IF(FTIP&1024=1024,"Bk","")) AS `Co`, ' + //GROUP_CONCAT("Nr. ", NRDFC, " / ", DATA,  " - ", SUMA, " RON" ORDER BY DATADOC) AS DATE_DOC ' +
                   ...
                   'CONCAT(IF(FTIP&2=2,"C",""), IF(FTIP&4=4,"G",""), IF(FTIP&8=8,"S",""), IF(FTIP&16=16,"M",""), IF(FTIP&32=32,"F",""), IF(FTIP&64=64,"Gw",""), IF(FTIP&128=128,"Rc",""), IF(FTIP&256=256,"Ex",""), IF(FTIP&512=512,"Gr",""), IF(FTIP&1024=1024,"Bk","")) AS `Co`, ' +
                   '"' + CGest.Items[I] + '" AS FIRMA ' +
...
begin
  if Options.Values['NR_FACT'] <> '' then begin
    CGestChange(Self);
...
begin
  ...
  with Ini do begin
    ...
    WriteBool('Misc', 'CCU0', Options.Values['CU_ZERO'] = 'TRUE');
    WriteBool('Misc', 'CBContract', Options.Values['CU_CONTRACT'] = 'TRUE');

    WriteInteger('Colors', 'Header',    Colors.HeaderColor);
    WriteInteger('Colors', 'FormColor', Colors.FormColor);
    WriteInteger('Colors', 'Footer',    Colors.FooterColor);
    WriteInteger('Colors', 'GridTitle', Colors.GridTitle);
    WriteInteger('Colors', 'Selection', Colors.Selection);
    WriteInteger('Colors', 'Cursor',    Colors.Cursor);
    WriteInteger('Colors', 'Odd',       Colors.Odd);
    WriteInteger('Colors', 'Even',      Colors.Even);
    WriteInteger('Colors', 'Contract',  Colors.Contract);
    WriteInteger('Colors', 'Overdue',   Colors.Overdue);
    WriteInteger('Colors', 'Blocked',   Colors.Blocked);
    WriteString('Colors', 'ThemeName', SM.SkinName);
    ...
    WriteString('Misc','NrFact', Options.Values['NR_FACT']);
  end;
  Options.Free;
end;
...
begin
  ...
  with Ini do begin
    ...
    Options.Values['FORCE_UPDATE'] := ReadString('Misc', 'ForceUpdate', '0');
    Options.Values['NR_FACT'] := ReadString('Misc', 'NrFact', '4');
    WriteString('Misc', 'LastRun', MySQLDateToStr(Now, 'YYYY-MM-DD'));

    //FormColors
    Colors.HeaderColor := ReadInteger('Colors', 'Header', 16758711);
    Colors.FormColor := ReadInteger('Colors', 'FormColor', 16758711);
    Colors.FooterColor := ReadInteger('Colors', 'Footer', 16758711);
    //GridColors
    Colors.GridTitle := ReadInteger('Colors', 'GridTitle', 16744576);
    Colors.Selection := ReadInteger('Colors', 'Selection', 12615680);
    Colors.Cursor := ReadInteger('Colors', 'Cursor', 12936525);
    Colors.Odd := ReadInteger('Colors', 'Odd', 14847867);
    Colors.Even := ReadInteger('Colors', 'Even', 14853526);
    //PartnerColors
    Colors.Contract := ReadInteger('Colors', 'Contract', 13105882);
    Colors.Overdue := ReadInteger('Colors', 'Overdue', 10528255);
    Colors.Blocked := ReadInteger('Colors', 'Blocked', 10528255);

    Options.Values['THEME_NAME'] := ReadString('Colors', 'ThemeName', '');
    SM.SkinName := Options.Values['THEME_NAME'];

    //pbSettings.Color := Colors.HeaderColor;
    MainForm.Color := Colors.FormColor;

    Options.Values['BROWSER'] := ReadString('Misc', 'Browser', '1');
    ...
    Options.Values['UPDATE_PATH'] := ReadString('Locations', 'Update', '\\alex\shared\blocare\blocare.exe');
    ...
    MainForm.Caption := 'dsBlocare - Sunteti logat ca: ' + UMail + '.  Data selectata este: ' + LowerCase(Luni[s2i(Copy(SDate, 1, 2))]) + ' 20' + Copy(SDate, 3, 2);
    DoRefresh := ReadInteger('Startup', 'Refresh', 0);
    DoExpire := ReadInteger('Startup', 'Expire', 0);
    DoVerify := ReadInteger('Startup', 'Verify', 0);
    ...
    Options.Values['CU_ZERO'] := iif(ReadBool('Misc', 'CCU0', True) = True, 'TRUE', 'FALSE');
    Options.Values['CU_CONTRACT'] := iif(ReadBool('Misc', 'CBContract', True) = True, 'TRUE', 'FALSE');
  end;
  CGest.Items.CommaText := GestDB;
  cbFirme.ItemIndex := CGestIndex;
  CGest.ItemIndex := CGestIndex;
  GestCurIndex := True;
  GestFieldCount := 6; //-1
  ManagerFieldCount := 7; //-1
  GestColWidths := Chr(70) + Chr(195) + Chr(16) + Chr(12) + Chr(12) + Chr(50) + Chr(61) + Chr(13) + Chr(42) + Chr(32);
  ManagerColWidths := Chr(1) + Chr(180) + Chr(170) + Chr(70) + Chr(45) + Chr(1) + Chr(1) + Chr(1);
  Keys.Clear;
  ...
  if Alias <> '' then begin
    ...
    Contract := iif(Options.Values['CU_CONTRACT'] = 'TRUE', 'AND ((CONT_USD <> "") OR (CONT_DM <> ""))', '');
    //
    HavingFact := ' ';
    ...
    UnionAll := 'UNION ALL ' +
                ...
                '0 AS `Sold`, " " AS `Data`, CLASA AS `L`, CONCAT(IF(FTIP&2=2,"C",""), IF(FTIP&4=4,"G",""), IF(FTIP&8=8,"S",""), ' +
                  ...
                  'IF(FTIP&512=512,"Gr",""), IF(FTIP&1024=1024,"Bk","")) AS `Co` ' +
               // '" " AS DATE_DOC ' +
                'FROM `' + CGest.Text + '`.`FUR` ' +
                'WHERE (CODFUR NOT IN ' +
                  '(SELECT CODFC FROM `' + CGest.Text + '`.`FRES' + SDate + '` ' +
...
procedure TMainForm.DoSchedule(Options: string);
begin
  if Options = 'F' then begin //first run in the morning
    if (DoExpire = 0) and ManagerVisible then ScheduleExpire(10);
    if DoRefresh = 0 then ScheduleRefresh;
    if DoVerify = 0 then ScheduleVerify;
  end;
  if Options = 'T' then begin //timer
    if (DoExpire = 1) and ManagerVisible then ScheduleExpire(10);
    if DoRefresh = 1 then ScheduleRefresh;
    if (DoVerify = 1) and GestVisible and (MainForm.Visible = True) then
      if not ActionExecuted then begin
        ActionExecuted := True;
        if (FDialog.Start('Verificare stare clienti','Doriti verificare stare?', fdBtn_Yes or fdBtn_No, mtConfirmation) = fdBtn_Yes) then ScheduleVerify;
      end;
  end;
  if Options = 'UM' then begin//Update
    TUpdateThread.Create(Application.ExeName,'M');
  end;
  if Options = 'UA' then begin//Update
    TUpdateThread.Create(Application.ExeName,'A');
  end;
end;
...
begin
  ...
  for I := 0 to CGest.Items.Count - 1 do
    SQL := SQL +
      ...
      'UNION ALL ';
  SQL := Copy(SQL, 1, Length(SQL) - 11) + ' ORDER BY DENFC ASC, EMAIL) T WHERE CONCAT(EMAIL,TEL) <> "" GROUP BY CODFC, EMAIL, TEL ORDER BY DENFC';
  QTemp.SQL.Text := SQL;
  QTemp.ExecSQL;
  ...
  SQL := Copy(SQL, 1, Length(SQL) - 11) + ') T GROUP BY EMAIL, TEL';
  QTemp.SQL.Text := SQL;
  QTemp.ExecSQL;
end;

procedure TMainForm.ScheduleVerify;
var
  SQL: string;
  I: Integer;
begin
  for I := 0 to CGest.Items.Count - 1 do
    SQL := SQL + //F - trebuie facturat, D - trebuie deblocat
           'SELECT CODFUR `Cod Fiscal`, CONCAT(DENFUR, " - ' + CGest.Items[I] + '") AS `Denumire`, CONTRACT `$`, SECTIA AS `St.`, ' +
             'NR_FACT AS `#`, SUM(IFNULL(REST_PLATA,0)) `Sold`, CONCAT(MID(DATA,7,2),".",MID(DATA,5,2),".",MID(DATA,1,4)) `Data`, CLASA `L`, ' +
             'CONCAT(IF(PER > CLASA,"F",""), IF((PER <= CLASA) AND (NR_FACT < ' + Options.Values['NR_FACT'] + '),"D","")) `Act`, "' + CGest.Items[I] + '" FIRMA ' +
           'FROM (' +
             'SELECT DENFUR, CODFUR, REST_PLATA, DATA, COUNT(CODFUR) NR_FACT, PERIOD_DIFF(DATE_FORMAT(now(),"%Y%m"),MID(DATA,1,6)) PER, ' +
               'CLASA, SUM(CLASA) NRLUNI, SECTIA, CONTRACT ' +
             'FROM (' +
               'SELECT U.CODFUR, DENFUR, CLASA, SECTIA, CONCAT(CONT_DM, CONT_USD) CONTRACT, ' +
                 'IF((@DATA_FUR := CONCAT(MID(TELEX,5,4),MID(TELEX,3,2),MID(TELEX,1,2))) > (DATA_UMIV), @DATA_FUR, DATA_UMIV) DATA ' +
               'FROM `' + CGest.Items[I] + '`.`FUR` F LEFT JOIN (' +
                 'SELECT CODFUR, CONCAT(MID(DATA,5,4),MID(DATA,3,2),MID(DATA,1,2)) DATA_UMIV ' +
                 'FROM `' + CGest.Items[I] + '`.`UMIV` ORDER BY CONCAT(MID(DATA,5,4),MID(DATA,3,2),MID(DATA,1,2)) DESC ) U ' +
               'USING(CODFUR) ' +
               'WHERE (CONT_USD <> 0 OR CONT_DM <> 0) ' +
               'GROUP BY F.CODFUR ' +
               'ORDER BY DATA DESC) FUR ' +
             'LEFT JOIN (' +
               'SELECT CODFC, (SUMADB-SUMACR) REST_PLATA, DATAFC ' +
               'FROM `' + CGest.Items[I] + '`.`FRES' + SDate + '` WHERE CONTFC LIKE "411%" AND SUMADB-SUMACR <> 0) FRES ' +
             'ON CODFUR = CODFC ' +
             'WHERE CODFUR IS NOT NULL ' +
             'GROUP BY CODFUR ' +
             'ORDER BY DENFUR) TEMP ' +
           'GROUP BY CODFUR ' +
           'UNION ALL ';
  SQL := Copy(SQL, 1, Length(SQL) - 11) + ' ORDER BY DENUMIRE';
  DBGest.Tag := 2;
  QGest.SQL.Text := SQL;
  QGest.Active := True;
  QGestRec := QGest.RecordCount;
  ...
  GestResize;
  DBGest.Columns[DBGest.Columns.Count - 1].Visible := False;
  LGest.Caption := IntToStr(QGest.RecordCount) + ' înregisträri.';
  DBGest.SetFocus;
...
begin
  ...
  if Key = VK_F3 then ChangeDate;
  if Key = VK_F5 then FEditare.Start('Editare','');
  if Key = VK_F10 then Sesizari.Start('');
  if (Key = VK_F11) and not (ssCtrl in Shift) then begin
    Culori.Start;
    DBGest.Repaint;
    DBManager.Repaint;
  end;
...
begin
  btnExit.Left := MainForm.ClientWidth - btnExit.Width - 3;
  btnMFilter.Left := MainForm.ClientWidth - btnMFilter.Width - 3;
  if GestVisible then begin
    ...
    //Up
    btnFilter.Left := PGest.Width - btnFilter.Width - 3;
    //Down
    LTotal.Left := PGest.Width - LTotal.Width - 3;
    LSelected.Left := LTotal.Left;
  end;
  if ManagerVisible then begin
    if GestVisible then begin
      PGest.Width := ClientWidth div 2;
    ...
    //Up
    LGrupuri.Left := PManager.Left + 8;
    CGroups.Left := PManager.Left + 130;
    //Down
    LManager.Left := PManager.Left + 3;
  end;
end;

procedure TMainForm.FormShow(Sender: TObject);
begin
  FontColor := SM.gd[SM.GetSkinIndex('EDIT')].FontColor[1];
  BkColor := SM.gd[SM.GetSkinIndex('EDIT')].Color;
  MedianColor := GetMedianColor(FontColor, BkColor, 30);

  if Options.Values['FIRST_RUN'] = 'D' then begin
    DoSchedule('UA');
    Options.Values['FIRST_RUN'] := 'N';
  end;
  if Options.Values['CU_CONTRACT'] = 'TRUE' then begin
    btnContract.Down := True;
    btnContract.Font.Style := [fsBold];
  end else begin
    btnContract.Down := False;
  ...
  if Options.Values['CU_ZERO'] = 'TRUE' then begin
    btnZero.Down := True;
    btnZero.Font.Style := [fsBold];
  end else begin
    btnZero.Down := False;
    btnZero.Font.Style := [];
  end;
  
  if Options.Values['CU_BLOCATI'] = 'TRUE' then begin
    btnCuBlocati.Down := True;
    btnCuBlocati.Font.Style := [fsBold];
  end else begin
    btnCuBlocati.Down := False;
  ...
  btnFilter.Hint := 'Filtreaza doar clientii care au cel putin X facturi restante (nr. fact. restante se completeaza in casuta  "Nr fact" din meniul Setari).'#13 +
                   'Tinand apasata tasta SHIFT puteti obtine un filtru doar cu cei blocati.';
  if MySQLDateToStr(Now, 'YYYY-MM-DD') > LastDate then begin
    DoSchedule('F');
    LastDate := MySQLDateToStr(Now, 'YYYY-MM-DD');
  end;
end;
...
procedure TMainForm.IUpMouseEnter(Sender: TObject);
begin
  IUpOver.Visible := True;
  IUp.Visible := False;
end;

procedure TMainForm.IUpOverClick(Sender: TObject);
begin
  IUp.Visible := False;
  IUpOver.Visible := False;
  pSettings.Top := pDown.Top - pSettings.Height - PManager.Top;
  pSettings.Visible := True;
  IDown.Visible := True;
  IDownOver.Visible := False;
end;

procedure TMainForm.IUpOverMouseLeave(Sender: TObject);
begin
  if pSettings.Visible = False then begin
    IUpOver.Visible := False;
    IUp.Visible := True;
  end;
end;

procedure TMainForm.LCopyRightMouseEnter(Sender: TObject);
begin
  LCopyRight.Hint := LCopyRight.Caption;
  LCopyRight.Caption := GetVersionString(Application.ExeName);
end;

procedure TMainForm.LCopyRightMouseLeave(Sender: TObject);
begin
  LCopyRight.Caption := LCopyRight.Hint;
end;

procedure TMainForm.LPanelMouseEnter(Sender: TObject);
var
...
begin
  for I := 0 to ComponentCount - 1 do
    if Components[I] is TImage then
      if LeftStr(Components[I].Name,5) = 'IOver' then
        TImage(Components[I]).Visible := False;
  AComponent := FindComponent('IOver'+ i2s(TLabel(Sender).Tag));
  if AComponent <> nil then TLabel(AComponent).Visible := True;
end;
...
procedure TMainForm.ImgPanelMouseEnter(Sender: TObject);
var
  AComponent: TComponent;
  I: integer;
begin
  for I := 0 to ComponentCount - 1 do
    if Components[I] is TImage then
      if LeftStr(Components[I].Name,5) = 'IOver' then
        TImage(Components[I]).Visible := False;
  AComponent := FindComponent('IOver'+ i2s(TImage(Sender).Tag));
  if AComponent <> nil then TImage(AComponent).Visible := True;
end;

procedure TMainForm.IDownMouseEnter(Sender: TObject);
begin
  IDownOver.Visible := True;
  IDown.Visible := False;
end;

procedure TMainForm.IDownOverClick(Sender: TObject);
var
  I: integer;
begin
  IDown.Visible := False;
  IDownOver.Visible := False;
  I := 1;
  while I + pSettings.Top <= pDown.Top do begin
    pSettings.Top := pSettings.Top + I;
    I := I * 2;
    Sleep(50);
...
procedure TMainForm.IDownOverMouseLeave(Sender: TObject);
begin
  if pSettings.Visible = True then begin
    IDownOver.Visible := False;
    IDown.Visible := True;
  end;
end;
...
procedure TMainForm.IFilterOMouseLeave(Sender: TObject);
begin
  TImage(Sender).Visible := False;
end;
...
begin
  ...
  if OnObject = 'IFilterO' then Btn_FiltruClick(Self);
  if OnObject = 'IMFilterO' then Btn_MFiltruClick(Self);

  if OnObject = 'CuBlocati' then CBlocatiClick(Self);
  if OnObject = 'Filtru2' then Btn_2LuniClick(Self);
  if OnObject = 'Date' then ChangeDate;
  if OnObject = 'Password' then AddModify.Start('PAROLA');
  if OnObject = 'Sesizari' then Sesizari.Start('');
  if OnObject = 'Verify' then ForceVerify(Self);
  
  if OnObject = 'Settings' then begin
    if Optiuni.Start() <> 2 then
      DoDBGrids(Self);;
  end;
end;
...
//  TextWidth: integer;
begin
  GradientHorizontalFooter(TPaintBox(Sender),i2s(ClientWidth div 2));
 { pbDown.Canvas.Brush.Style := bsClear;
  pbDown.Canvas.Font := LFact.Font;
  pbDown.Canvas.TextOut(LFact.Left, LFact.Top, LFact.Caption);
  pbDown.Canvas.Font := LFact.Font;
  pbDown.Canvas.TextOut(LFact.Left, LFact.Top, LFact.Caption);
  pbDown.Canvas.Font := LTotal.Font;
  TextWidth := pbDown.Canvas.TextWidth(LTotal.Caption);
  pbDown.Canvas.TextOut(LTotal.Left - LTotal.Width + TextWidth, LTotal.Top, LTotal.Caption);
  pbDown.Canvas.Font := LGest.Font;
  pbDown.Canvas.TextOut(LGest.Left, LGest.Top, LGest.Caption);
  pbDown.Canvas.Font := LSelected.Font;
  TextWidth := pbDown.Canvas.TextWidth(LSelected.Caption);
  pbDown.Canvas.TextOut(LSelected.Left - LSelected.Width + TextWidth, LSelected.Top, LSelected.Caption);
  pbDown.Canvas.Font := LManager.Font;
  pbDown.Canvas.TextOut(LManager.Left, LManager.Top, LManager.Caption);
  pbDown.Canvas.Font := LMSelected.Font;
  TextWidth := pbDown.Canvas.TextWidth(LMSelected.Caption);
  pbDown.Canvas.TextOut(LMSelected.Left - LMSelected.Width + TextWidth, LMSelected.Top, LMSelected.Caption);   }
end;
...
begin
  ...
  {pbToolbar.Canvas.Brush.Style := bsClear;
  pbToolbar.Canvas.Font := LDBGest.Font;
  pbToolbar.Canvas.TextOut(LDBGest.Left, LDBGest.Top, LDBGest.Caption);
  pbToolbar.Canvas.Font := LGrupuri.Font;
  pbToolbar.Canvas.TextOut(LGrupuri.Left, LGrupuri.Top, LGrupuri.Caption);

  pbToolbar.Canvas.Font := LFilter.Font;
  pbToolbar.Canvas.TextOut(LFilter.Left, LFilter.Top, LFilter.Caption);
  pbToolbar.Canvas.Font := LMFilter.Font;
  pbToolbar.Canvas.TextOut(LMFilter.Left, LMFilter.Top, LMFilter.Caption);  }
end;
...
begin
  ...
  tsFiles := TStringList.Create;
  try
    SQLOpen(MyQuery, 'SELECT ATACHMENTS FROM DBCABCONTA.SENTMAIL WHERE ID = ' + i2s(LastMailID));
    if MyQuery.RecordCount <> 0 then
      tsFiles.CommaText := SQLString(MyQuery.FieldByName('ATACHMENTS').AsString, 'DECODE');
      for I := 0 to tsFiles.Count - 1 do begin
        Aux := Copy(tsFiles[I], Pos('|', tsFiles[I]) + 1);
        if Aux = '' then Aux := tsFiles[I];
        if FileExists(Aux) and (Copy(ExtractFileName(Aux), 1, 9) = 'TempFile_') then DeleteFile(Aux);
      end;
  except
    FDialog.Start('Atentie', 'Eroare la stergerea fisierelor temporare!', fdOk);
  end;
  tsFiles.Free;

  MyQuery.SQL.Text := 'UPDATE `DBCABCONTA`.`SENTMAIL` SET SENT = "D", SENTAT = NOW() WHERE ID = ' + IntToStr(LastMailID);
  MyQuery.ExecSQL;
  if (Assigned(SentMail)) then
    if SentMail.Visible then
      SentMail.FormShow(SentMail);
  MyQuery.Free;
end;
