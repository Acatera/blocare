...
uses
  ...
  StdCtrls, IdCoder, IdCoder3to4, IdCoderMIME;
...
uses Sesiz_Detail, FMain, Dialog;
...
begin
  ...
  S := StringReplace(S, '||GLM||', '"', [rfReplaceAll]);
  ...
  if DBGrid.Fields[6].AsString = StringReplace(UpperCase(Copy(UMail, 1, Pos('@', UMail) - 1)), 'TEO', 'TEODORA', [rfReplaceAll]) then
    DBGrid.Canvas.Brush.Color := Colors.Contract;
