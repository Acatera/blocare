...
begin
  ...
  if CompareVersions(GetVersion(PMyName), GetVersion(UPDATE_PATH)) or (MainForm.Options.Values['FORCE_UPDATE'] = '1') then begin
    if FDialog.Start('Actualizare', 'Alex a mai lucrat ceva la Blocare.'#13 +
                                    'Actualizezi?'#13 +
                                    #13 +
                                    'Versiune curenta:' + GetVersionString(PMyName) + #13 +
                                    'Versiune actualizare:' + GetVersionString(UPDATE_PATH), fdBtn_Yes or fdBtn_No, mtConfirmation, False) = fdBtn_Yes then begin
