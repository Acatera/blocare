...
begin
  ...
  QTemp.SQL.Text :=
    ...
    'ORDER BY CONCAT(MID(DATAFC,5,4),MID(DATAFC,3,2),MID(DATAFC,1,2))';
  QTemp.Open;
  for I := 0 to Selection.Count - 1 do begin
    if QTemp.Locate('NRDFC',Selection[I],[loCaseInsensitive]) then begin
      ...
      //Ramas - cat mai trebuie repartizat
      //Achitat - Suma ce se achita din factura
      //DB-CR - Ramas de plata
  ...
  if not Eroare then begin
    ...
    try
      ...
      if Result.NrOP = 0 then
        Result.NrOP := Result.Ref mod 100;
    except
      Eroare := True;
    end;
  end;
...
begin
  if btnAuthPayment.Tag = 0 then begin
    ...
    QTemp.SQL.Text :=
      'SELECT SECTIA ' +
      'FROM `' + sFirma + '`.`FUR` ' +
      'WHERE (CODFUR = "' + sCodFC + '") AND (SECTIA="B")';
    QTemp.Open;
    ...
    Q.Close; //I like this, this is elegant. 
    Q.Open;
    DBG.SetFocus;
